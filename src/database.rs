...
pub async fn get_commissions_orders(
    pool: &sqlx::MySqlPool,
) -> Result<Vec<ce_models::Order>, Box<dyn std::error::Error>> {
    //Get the orders
    let orders =
        sqlx::query!("SELECT * FROM Orders WHERE locked_date IS NULL AND is_commissionable = 1;",)
            .map(|row| {
                Box::pin(async move {
                    let order_date = row.order_date.map(convert_to_naive_date_time).unwrap();
                    let shipped_date: Option<NaiveDateTime> =
                    ...
                    ce_models::Order {
                        ...
                        city_local_shipping_tax: f64::from(row.city_local_shipping_tax.unwrap()),
                        county_shipping_tax: f64::from(row.county_shipping_tax.unwrap()),
                        county_local_shipping_tax: f64::from(
                            row.county_local_shipping_tax.unwrap(),
                        ),
                        other11: row.other11,
                        other12: row.other12,
                        other13: row.other13,
